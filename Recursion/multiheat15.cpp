...
struct coordinates{
  ...
  bool grid[15][15]; //let's store the fullness information in this part
};
...
int H = 15; // size of grid
int T = 3; // number of small particles
int Bs = 4; // size of Big
int Ss = 2; // size of Small
int Limit = 15; //limit number of outputs
int Heaters = 4; // number of heatmaps
int Heatmap[4][15][15]; //heatmap the position of the left hand corner of big particle
...
bool valid(coordinates &c, int x , int y, int size ){
  if(y-(size-1) > -1 && x+(size-1) < H){ // make sure it is not too large for box
